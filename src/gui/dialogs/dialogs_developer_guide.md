...
- `prompt_injection_dialog.py`: Prompt injection configuration dialogs. `PromptTemplateManagerDialog` manages saved templates and default enabled state. `PromptInjectionRunDialog` configures template toggles plus one-off text for the next run only.
...
- Prompt injection dialogs style editable lists and text editors for template content and one-off context input.

## When to Edit
- Adjust usage/rate-limit dialog layout or messaging: `usage_limit_dialog.py`.
- Adjust template CRUD and next-run injection controls: `prompt_injection_dialog.py`.
