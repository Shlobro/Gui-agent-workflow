...
- `properties_panel.py`: Resizable side panel with `_OverviewForm` (page 0), `_LLMForm` (page 1), `_FileOpForm` (page 2), `_ConditionalForm` (page 3), `_LoopForm` (page 4), `_JoinForm` (page 5), `_GitActionForm` (page 6), and `_AttentionForm` (page 7). LLM form uses Prompt/Output tabs, where Prompt contains prompt edit + preview and Output contains the execution log. Exposes `show_overview()` and `set_overview_text(...)` for no-selection state. Emits node edit signals and `text_zoom_changed`.
