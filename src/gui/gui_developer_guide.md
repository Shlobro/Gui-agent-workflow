...
- `bubble_node.py`: `BubbleNode` — draggable node with embedded editor, model selector, and output panel. `StartNode` — permanent pure-trigger root node with output port only. `_GlowAnimator` singleton drives the animated "running" border effect (~33 fps QTimer, shared across all running nodes).
- `bubble_widget.py`: `BubbleWidget` (embedded QWidget inside BubbleNode), `ModelSelector` (viewport-anchored dropdown), and `_ModelListWidget`. `ModelSelector` emits `model_changed(old_id, new_id)` when the user picks a different model.
- `connection_item.py`: Directed Bezier edge between bubble ports.
- `undo_commands.py`: `QUndoCommand` subclasses for every graph mutation (add/remove bubble, add/remove connection, move, rename, model change, paste).
...
- Running nodes (status `"running"`) display an animated border: a pulsing outer halo and a rotating bright-spot sweep implemented via `QConicalGradient` driven by `_GlowAnimator`. The timer starts when the first node enters `"running"` and stops when none remain running.
...
- **Copy/paste**: Ctrl+C snapshots all selected `BubbleNode`s (Start excluded) and any connections whose both endpoints are in the selection into an in-memory clipboard (`_clipboard` / `_clipboard_conns`), and resets a `_paste_count` counter to 0. Ctrl+V pushes a `PasteCommand` onto the undo stack; the command creates each node with a fresh `uuid4` bubble ID and a pre-assigned `_bubble_counter` label index; position is the original coordinates plus `40 * _paste_count` on both axes (incremented each paste, so successive pastes of the same clipboard land at distinct cascading offsets). Internal connections are recreated between the new nodes. Pasted nodes are selected and originals are deselected. Both shortcuts are suppressed when any embedded node widget has focus (via `QGraphicsProxyWidget` scene focus) so normal text editing is unaffected. The clipboard persists only for the session and is not saved with workflow JSON.
- **Undo/redo**: Every graph mutation (add bubble, delete bubble, add connection, delete connection, drag move, rename title, change model, paste) is wrapped in a `QUndoCommand` and pushed onto `WorkflowCanvas._undo_stack` (limit 100). Ctrl+Z calls `undo()` and Ctrl+Y calls `redo()` — both shortcuts are suppressed when any `QGraphicsProxyWidget` has scene focus (i.e. the user is interacting with any embedded node widget, including the prompt editor, title field, or model selector), so native widget undo still works inside text fields. Toolbar Undo/Redo buttons show the current command name and are enabled/disabled automatically by Qt. Multi-item deletes and multi-node drags are wrapped in a macro so they undo as one step. `_undo_stack.clear()` is called on `clear_canvas()` and `load_workflow_data()` so stale history cannot refer to deleted nodes. The `_undo_in_progress` flag prevents re-entrancy when `undo()`/`redo()` programmatically sets model or title, which would otherwise push a new command. Raw (no-stack) node manipulation helpers (`_undo_add_bubble`, `_undo_remove_bubble`, `_undo_add_connection`, `_undo_remove_connection_item`) are called only by command objects.
