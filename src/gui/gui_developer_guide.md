...
- `bubble_node.py`: `BubbleNode` — draggable node with embedded editor, model selector, and output panel. `StartNode` — permanent pure-trigger root node with output port only. `ModelSelector` emits `model_changed(old_id, new_id)` when the user picks a different model.
