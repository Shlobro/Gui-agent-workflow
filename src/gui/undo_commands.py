...
class AddBubbleCommand(QUndoCommand):
    ...
    def redo(self):
        self._canvas._undo_add_bubble(self._snapshot, self._label_index)

    def undo(self):
        self._canvas._undo_remove_bubble(self._snapshot["id"])
...
class RemoveBubbleCommand(QUndoCommand):
    ...
    def undo(self):
        node = self._canvas._undo_add_bubble(self._snapshot, self._label_index)
        if node is None:
            return
        for c in self._conn_snapshots:
            ...
            if src_id == "start":
                ...
                src = self._canvas._bubbles.get(src_id)
            if tgt_id == "start":
                tgt = self._canvas._start_node
            else:
                tgt = self._canvas._bubbles.get(tgt_id)
            if src and tgt:
...
class AddConnectionCommand(QUndoCommand):
    ...
    def _resolve(self):
        src = (self._canvas._start_node if self._src_id == "start"
               else self._canvas._bubbles.get(self._src_id))
        tgt = (self._canvas._start_node if self._tgt_id == "start"
               else self._canvas._bubbles.get(self._tgt_id))
        return src, tgt
...
class MoveBubbleCommand(QUndoCommand):
    ...
    def mergeWith(self, other: QUndoCommand) -> bool:
        if not isinstance(other, MoveBubbleCommand):
            return False
        if other._bubble_id != self._bubble_id:
            return False
...
class TitleChangeCommand(QUndoCommand):
    ...
    def redo(self):
        ...
        if node:
            ...
            self._canvas._title_committed[self._bubble_id] = self._new_title
    ...
    def undo(self):
        ...
        if node:
            ...
            self._canvas._title_committed[self._bubble_id] = self._old_title
            self._canvas._undo_in_progress = False
...
class PasteCommand(QUndoCommand):
    ...
    def __init__(self, canvas: "WorkflowCanvas", clipboard: List[dict],
        ...
        for data in clipboard:
            ...
            snap["label_index"] = canvas._bubble_counter
            self._node_snapshots.append(snap)
            self._label_indices.append(canvas._bubble_counter)

    def redo(self):
        new_nodes = []
        for snap, li in zip(self._node_snapshots, self._label_indices):
            node = self._canvas._undo_add_bubble(snap, li)
            if node:
    ...
    def undo(self):
        for snap in self._node_snapshots:
            self._canvas._undo_remove_bubble(snap["id"])
