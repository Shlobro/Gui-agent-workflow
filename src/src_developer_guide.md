...
## Data Flow
1. UI collects node configuration through `PropertiesPanel`. The panel stays visible at all times: with one selected node it shows node-edit forms, otherwise it shows a workflow overview summary (counts, invalid nodes, working directory, prompt injection payload). Content fields commit directly to node attributes on focus loss or explicit commit, while title, model, op-type, condition-type, loop-count, and git-action changes go through canvas undo handlers. `PropertiesPanel.commit_pending_edits()` is invoked before run/save and before panel node switches so canvas state stays in sync. `MainWindow` persists panel width and panel-local text zoom in `QSettings`. For selected LLM nodes, the panel also shows a live read-only Prompt Preview that uses the same template/one-off composition as execution and stays pinned to active run settings while a run is in progress.
...
3. Canvas offers three run modes: Run All, Run Selected, and Run From Here. Pre-run validation requires: prompt plus resolvable model for `LLMNode`; filename for `FileOpNode`; known `condition_type` and a filename only for filename-scoped conditions on `ConditionalNode`; non-empty message for `AttentionNode`; valid git enums plus commit-message requirements for `GitActionNode`. `LoopNode` needs no extra validation because `loop_count` is validated at parse/load time. The same rules also drive live validity markers (`WorkflowCanvas.refresh_node_validation_state()`): invalid nodes render with a red border until required fields are fixed.
4. `LLMNode` execution uses `LLMWorker` in a background `QThread`. Before worker start, prompt text is assembled through `src.llm.prompt_injection.compose_prompt(...)` using template content configured in `MainWindow` (default-enabled templates or next-run overrides), with per-template and one-off placement on the prepend/append side of the base prompt. `GitActionNode` execution uses `GitWorker` in a background `QThread` with streaming output, cancellation, and watchdog timeout handling. `FileOpNode`, `AttentionNode`, and `LoopNode` execute synchronously on the GUI thread. Loop execution uses status `looping` while in progress, routes the `loop` branch exactly `loop_count` times for each loop thread, and routes the `done` branch on the next loop-node re-entry, so loop bodies need an explicit feedback edge back into the loop input. `ConditionalNode` execution is mixed: file-based evaluators stay synchronous, while `git_changes` runs `git status --porcelain --untracked-files=all` against the selected project folder in a background `GitWorker` with live streamed git output, a 15-second timeout, and a clear timeout error if git does not return in time. Child triggering is deferred with `QTimer.singleShot(0, ...)`. Trigger calls carry both `lineage_token` and `loop_token`. LLM failure handling also classifies Claude CLI quota output in the form `You've hit your limit · resets ...`, OpenAI-style `rate limit reached for requests` / `You exceeded your current quota`, Anthropic-style `rate_limit_error`, and Gemini-style `RESOURCE_EXHAUSTED`, `QUOTA_EXHAUSTED`, `TerminalQuotaError`, or `You have exhausted your capacity on this model` as usage-limit events so `MainWindow` can stop the run and offer the model-change dialog.
...
7. Streamed output lines and terminal node results are appended live to node state and surfaced in the properties panel output area for whichever node is currently selected. For LLM nodes, prompt/output share a vertical splitter, and the prompt editor area includes a read-only preview below the prompt showing the fully composed prompt (plain concatenated text without section labels).
...
9. Prompt injection templates are persisted in repo-root `.prompt_injections.json` through `src.llm.prompt_injection.PromptInjectionStore`. Template records include persistent placement (`prepend` or `append`), and built-in template placement is persisted as well. The built-in headless runtime template is always available and enabled by default on first load.
