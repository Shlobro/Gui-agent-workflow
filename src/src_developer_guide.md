...
9. Prompt injection templates are persisted in repo-root `.prompt_injections.json` through `src.llm.prompt_injection.PromptInjectionStore`. The built-in headless runtime template is always available and enabled by default on first load.
