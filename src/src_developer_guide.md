...
4. `LLMNode` execution uses `LLMWorker` in a background `QThread`. Prompt text is assembled through `src.llm.prompt_injection.compose_prompt(...)` using template content configured in `MainWindow`. Claude and Codex providers use structured CLI output so the worker can capture resumable conversation IDs (`session_id` for Claude, `thread_id` for Codex); Gemini remains plain-text.
