...
7. When `resume_session_enabled` is checked on a Claude or Codex node, later calls reuse that node's saved session ID. Named-session resume pulls from the workflow-level store instead, but only when the target session already has a captured ID, matches the current provider, and the graph contains a directed path from the save-owner node to the current load node. A save-owner node can also persist `restart_session_enabled`, which makes that node ignore the currently saved resumable ID for its own call and overwrite the workflow-named session only after the fresh call finishes.
...
- Claude provider: Opus 4.8 and Sonnet 4.6. Opus 4.8 is exposed with `low`, `medium`, `high`, `xhigh`, and `max`; Sonnet 4.6 is exposed with `low`, `medium`, `high`, and `max`.
- Codex CLI / OpenAI provider: GPT-5.5, GPT-5.4, and GPT-5.3 Codex, each with `low`, default (medium), `high`, and `xhigh` effort options.
