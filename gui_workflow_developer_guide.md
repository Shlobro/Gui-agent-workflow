...
4. `src.gui.main_window.MainWindow` creates the menu bar, toolbar, status shell, and mounts `WorkflowCanvas`. The chosen project folder is set as the LLM/git subprocess `cwd` via `WorkflowCanvas.set_working_directory()`.
5. Canvas interactions create nodes/connections, support wheel zoom and right-drag panning, allow left-drag rubber-band multi-selection, render selected nodes with a light-blue neon frame, keep background grid dots visible with fixed on-screen size across zoom levels, auto-expand scene bounds so corner nodes remain reachable, route connection lines around node bounds (including immediate reroute right after connection creation), keep arrowheads visible by ending slightly before the input port center, show direction cues on ports with ingress/egress arrows that protrude from node edges and hide once that side is connected, expose an expanded connection hit area so arrow selection is easier, suspend wheel zoom while model dropdowns are open so wheel input scrolls the model list, copy selected nodes (Ctrl+C) and paste duplicates with fresh IDs at cumulative offsets (40 px × paste count, reset on each copy) preserving internal connections, undo/redo graph mutations (Ctrl+Z / Ctrl+Y) covering add/delete nodes, add/delete connections, node moves, title renames, model changes, file-op type changes, git action type changes, condition type changes, loop count changes, and paste operations — all Ctrl shortcuts suppressed while any text widget (QLineEdit, QPlainTextEdit, QTextEdit) has focus so native widget undo still works, and optionally save/load workflow JSON files. Nodes are compact painted items (64 px tall for LLM, file-op, and git-action nodes; 80 px tall for conditional and loop) showing only a colored header strip and title label; all editing is done in the sliding `PropertiesPanel` on the right side of the window. `ConditionalNode` has two output ports (true/false) on the right edge; `LoopNode` has two output ports (loop/done) on the right edge; connections from these ports carry a `source_port` field and the execution engine routes downstream triggers to only the matching branch.
6. Three run modes are available. **Run All**: starts from the permanent Start node, validates all reachable nodes (LLMNode requires prompt plus a model that resolves to a registered provider; FileOpNode requires filename; ConditionalNode requires filename and a known `condition_type`; LoopNode has no extra validation requirement; GitActionNode requires `git_action` in `{git_add, git_commit, git_push}` and `msg_source` in `{static, from_file}`, and `git_commit` also requires a non-empty commit message or commit message file depending on `msg_source`), then fires Start's direct children. LLM nodes execute in `LLMWorker`; git-action nodes execute in `GitWorker` with live streamed output lines and timeout enforcement that still triggers when commands are silent/hung. On non-zero git exits, the terminal error line is a concise summary (`git command failed (exit X)`) because detailed command output has already streamed line-by-line. When an LLM worker exits with a non-zero return code, the full accumulated output is the error payload; `execution.py` tests it against `_USAGE_LIMIT_RE` (covering Claude, Codex, and Gemini usage/rate-limit messages). A matching error calls `stop_all()` to cancel all parallel branches and emits `usage_limit_hit(node_id, error_text)` deferred via `QTimer.singleShot(0, ...)`. `MainWindow._on_usage_limit_hit` opens `UsageLimitDialog`; if the user clicks **Change Model**, the failed node is selected and the properties panel opens to its model selector. Generic LLM errors (command not found, network failure, etc.) keep the existing behavior: the branch stops, siblings continue, no dialog appears. File-op, conditional, and loop nodes execute synchronously on the UI thread with downstream fan-out queued through the Qt event loop. `stop_all()` cancels in-flight LLM and git workers. Deferred callbacks re-check that the source→target edge still exists before firing. Run completion waits for both active invocations and queued child triggers to drain. File-op paths must stay inside the selected project folder (absolute paths and traversal escapes are rejected). `create_file` reports `Already exists` for existing files and raises an error if the target path already exists as a directory/non-file path. **Run Selected**: fires only the selected node(s) simultaneously without any fan-out to children. **Run From Here**: fires the single selected node and propagates to all its descendants. `MainWindow` calls `PropertiesPanel.commit_pending_edits()` before all run modes and before Save Workflow; the panel also commits pending edits before switching to a different node. Load/Clear hide the panel before destructive canvas updates, commit handlers skip missing/non-live node IDs, and workflow load surfaces parse/schema errors through an error dialog instead of crashing. Workflow payload parsing is atomic: parse/validate first, then clear/apply only on success, so invalid files do not wipe the current canvas. A malformed node record (bad structure, failed deserialization, or duplicate ID) aborts the entire load; malformed connection records are silently dropped because a partial connection set is still a coherent graph. This keeps execution/serialization aligned with the latest visible panel edits and prevents stale panel commits from targeting replaced/deleted nodes. The Start-root restriction applies only to Run All.
...
- All LLM and git subprocess calls run with the selected project folder as their working directory (`cwd`).
...
- File operation node (create/truncate/delete file, op type set in panel, synchronous execution): `src/gui/file_op_node.py`.
- Conditional routing node (evaluates a condition, routes true/false branches): `src/gui/conditional_node.py`.
- Loop node (fires loop port N times then fires done port): `src/gui/loop_node.py`.
- Git action node (stores git action/message settings and renders git nodes): `src/gui/git_action_node.py`.
- Sliding properties panel and extracted form widgets (node name, model, prompt, file-op operation type, condition type, loop count, filename, git action/message settings, output): `src/gui/properties_panel.py`, `src/gui/_panel_forms.py`.
...
- Usage/rate-limit error detection and dialog: `src/gui/canvas/execution.py` (`is_usage_limit_error`, `usage_limit_hit` signal), `src/gui/dialogs/usage_limit_dialog.py`, `src/gui/main_window.py` (`_on_usage_limit_hit`).
